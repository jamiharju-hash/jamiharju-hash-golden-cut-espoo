...
function LocationCta({
  ...
  return (
    <a
      ...
      className={`inline-flex min-h-14 items-center justify-center gap-3 bg-amber-400 px-5 py-4 text-center text-[0.78rem] font-black uppercase tracking-[0.1em] text-black shadow-[0_18px_45px_rgba(251,191,36,0.18)] transition hover:-translate-y-0.5 hover:bg-amber-300 sm:min-h-[4.25rem] sm:px-8 sm:text-sm sm:tracking-[0.14em] ${className}`}
    >
      {label}
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-start justify-between gap-4">
        <Logo />
        <button onClick={onClose} className="grid h-12 w-12 shrink-0 place-items-center text-white sm:h-14 sm:w-14" aria-label="Sulje valikko">
      ...
      <div className="mx-auto grid max-w-7xl gap-12 pb-10 pt-16 sm:gap-16 sm:pt-24 lg:grid-cols-[1.08fr_0.92fr] lg:items-end lg:pb-0 lg:pt-32">
        <nav className="grid gap-7 sm:gap-10">
          {nav.map(([href, label], index) => (
            <motion.a
              ...
              onClick={onClose}
              className="font-display text-[clamp(2.7rem,14vw,7rem)] font-black leading-[0.9] tracking-[-0.075em] text-white/85 transition hover:text-amber-400"
            >
              {label}
            </motion.a>
          ))}
        ...
        >
          <div>
            ...
            <a href={emailHref} className="break-all text-[1.35rem] font-black uppercase leading-tight tracking-[-0.03em] text-white/85 sm:text-4xl">
              {email}
            </a>
          </div>
          ...
          <div>
            ...
            <a href={phoneHref} onClick={() => trackPhone("full_menu_phone")} className="text-2xl font-black text-white/85 sm:text-4xl">
              {phoneDisplay}
            </a>
          </div>
          ...
          <a
            ...
            className="inline-flex w-fit items-center gap-4 text-2xl font-black uppercase tracking-[0.03em] text-white transition hover:text-amber-400 sm:gap-5 sm:text-4xl"
          >
            Hae reitit
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:h-24 sm:px-6 lg:px-8">
          ...
          <button onClick={() => setOpen(true)} className="grid h-12 w-12 shrink-0 place-items-center text-white sm:h-14 sm:w-14" aria-label="Avaa valikko">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
              <div className="min-w-0 flex-1">
                <div className="flex items-start gap-4 sm:items-center">
                  <h4 className="text-xl font-black leading-tight text-white transition-colors group-hover:text-amber-400 sm:text-2xl">{service.name}</h4>
  ...
  return (
    <section id="kuvat" className="bg-black px-5 py-20 sm:px-6 sm:py-24 lg:px-8">
      <motion.div variants={reveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="mx-auto max-w-7xl">
        ...
        <a href={heroPhoto.sourceUrl} target="_blank" rel="noreferrer" onClick={() => trackMaps("gallery_photo") } className="mt-10 block overflow-hidden border border-white/10 sm:mt-12">
          ...
          <div id="yhteys" className="grid gap-8 lg:grid-cols-2 lg:gap-10">
            <a href={mapsUrl} target="_blank" rel="noreferrer" onClick={() => trackMaps("footer_location")} className="flex gap-4 border-t border-white/10 pt-6 sm:gap-5">
              ...
              <div className="min-w-0">
                ...
                <span className="mt-6 inline-flex items-center gap-3 text-xl font-black uppercase text-white transition hover:text-amber-400 sm:mt-7 sm:gap-4 sm:text-2xl">
